...
export default function Projects() {
  return (
      <div className='projects'>
        ...
        <Project img={libraryImg} name="Library Catalog" madeWith={"Made with: NodeJS, Express, MongoDB"} githubLink={"https://github.com/ccrutcher/express-locallibrary"} demoLink={"https://crutcher-express-library.herokuapp.com/"} description={"A Node.js project created with Express. This project uses Mongoose to model data that is then passed to the MongoDB database. The app utilizes the MVC architectural pattern to keep the code structured, with Pug functioning as the templating engine. The app has full CRUD capabilies, allowing users to create and interact with books, authors, genres, and book instances within the database."}></Project>
        <Project img={musicImg} name="Music Player" madeWith={"Made with: ReactJS"} githubLink={"https://github.com/ccrutcher/music-player"} demoLink={"https://crutcher-react-music-player.herokuapp.com/"} description={"From the landing page, using React Router, users may go to the library. Once an album has been selected, users can click on any song to begin playing the song, then clicking again will pause it."}></Project>
